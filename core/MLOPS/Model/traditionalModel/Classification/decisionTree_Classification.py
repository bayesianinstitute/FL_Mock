...
class DecisionTreeTabularClassification:
    def __init__(self, ip="http://localhost", port=5000, experiment_name='decision_tree_classification_experiment'):
        self.x_train, self.y_train, self.x_test, self.y_test = self.load_and_preprocess_data()

        # Start MLflow experiment
        self.name = "DecisionTree_" + datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        self.url = f'{ip}:{port}'
    ...
    def train_model(self,epochs=5):
        try:
            mlflow.start_run(run_name=f'{self.name}_DecisionTree')
            mlflow.autolog()
    ...
    def save_model(self, model_filename):
        try:
            ...
            model_path = f"mlruns/{datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}/{model_filename}"
...
if __name__ == '__main__':
    # Example usage:
    decision_tree_classification_model = DecisionTreeTabularClassification()
    decision_tree_classification_model.train_model()
    decision_tree_classification_model.save_model("decision_tree_model.joblib")
