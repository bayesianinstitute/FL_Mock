...
class TrainingInformation(models.Model):
    model_name_choices = [
        ...
        ('ANN-Classification','ANN-Classification'),
        ('LSTM-NLP','LSTM-NLP'),
        ('LSTM-TimeSeries','LSTM-TimeSeries'),
        ('RNN-TimeSeries','LSTM-TimeSeries'),
    ]
...
class Admin(models.Model):
    ...
    training_info = models.ForeignKey(
        ...
        related_name='training_results_admin',  # Provide a unique related_name
        null=True,
        blank=True
    )
...
class TrainingResultAdmin(models.Model):
    ...
    node_id = models.ForeignKey(
        Admin,
        on_delete=models.CASCADE,
        related_name='admin_training_node',  # Provide a unique related_name
        null=True,
        blank=True
    )
    accuracy = models.FloatField()
...
class Logs(models.Model):
    ...
    training_info = models.ForeignKey(
        TrainingInformation,
        on_delete=models.CASCADE,
        related_name='training_results_log',  # Provide a unique related_name
        null=True,
        blank=True
    )
    timestamp = models.DateTimeField(auto_now_add=True)
...
class GlobalModelHash(models.Model):
    ...
    training_info = models.ForeignKey(
        TrainingInformation,
        on_delete=models.CASCADE,
        related_name='training_results_global',  # Provide a unique related_name
        null=True,
        blank=True
    )
    timestamp = models.DateTimeField(auto_now_add=True) 
